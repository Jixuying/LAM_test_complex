...
class ComplexConv2d(Module):
    ...
    def __init__(self,in_channels, out_channels, kernel_size=3, stride=1, padding = 0,
        ...
        super(ComplexConv2d, self).__init__()#这里 ComplexConv2dji继承nn.Module中的init
        self.conv_r = Conv2d(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias)
        self.conv_i = Conv2d(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias)
        # torch.nn.init.kaiming_normal(self.conv_r.weight)
        # torch.nn.init.kaiming_normal(self.conv_i.weight)
    def forward(self,input_r, input_i):
...
class _ComplexBatchNorm(Module):
    ...
    def __init__(self, num_features, eps=1e-5, momentum=0.1, affine=True,
        ...
        if self.track_running_stats:
            # register_buffer向模块添加持久缓冲区。
            # 这通常用于注册不应被视为模型参数的缓冲区。例如，BatchNorm的running_mean不是一个参数，而是持久状态的一部分。
            # 缓冲区可以使用给定的名称作为属性访问。
            # 说明：应该就是在内存中定义一个常量，同时，模型保存和加载的时候可以写入和读出。
...
class ComplexBatchNorm2d(_ComplexBatchNorm):#继承了_ComplexBatchNorm

    def forward(self, input_r, input_i):
        assert(input_r.size() == input_i.size())#Python assert（断言）用于判断一个表达式，在表达式条件为 false 的时候触发异常。
        assert(len(input_r.shape) == 4)
        exponential_average_factor = 0.0


        if self.training and self.track_running_stats:#track_running_stats:一个布尔值，当设置为True时，此模块跟踪运行平均值和方差；设置为False时，此模块不跟踪此类统计信息，并且始终在训练和评估模式下使用批处理统计信息。默认值：True
        ...
        if self.training:
            ...
            mean_r = input_r.mean([0, 2, 3])#对0 2 3维的所有数字64*12*12进行求平均——出来20个数字
            ...
            # update running mean
            with torch.no_grad(): #之后的内容不需要进行图构建——grad_fn=<AddmmBackward>这个属性没有
                self.running_mean = exponential_average_factor * mean\
                    + (1 - exponential_average_factor) * self.running_mean  #算出来是20*2 后面这个2 是分为一个实部一个虚部

            input_r = input_r-mean_r[None, :, None, None]#none在ndarray和tensor类型的数组中适用，在数组索引中，加入None就相当于在对应维度加一维——那一维不会有真的数值
            input_i = input_i-mean_i[None, :, None, None]

            # Elements of the covariance matrix (biased for train)
            n = input_r.numel() / input_r.size(1) # numel()函数：返回数组中元素的个数/第一维的个数
            Crr = 1./n*input_r.pow(2).sum(dim=[0,2,3])+self.eps # 平方以后，那三维的数字（9120个）一起相加 /n ——估计的是有偏方差
            Cii = 1./n*input_i.pow(2).sum(dim=[0,2,3])+self.eps
            Cri = (input_r.mul(input_i)).mean(dim=[0,2,3]) #x.mul(y)或x.mul_(y)实现把x和y点对点相乘，其中x.mul_(y)是in-place操作，会把相乘的结果存储到x中
        ...
        if self.affine: #除了wight和bias bn的其他项不需要求导——也就是那些个均值和协方差是不需要求导的——所以计算的时候 with torch.no_grad()：
